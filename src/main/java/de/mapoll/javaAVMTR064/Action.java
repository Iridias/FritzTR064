...
public class Action {
    ...
    public Action(ActionType action, List<StateVariableType> stateVariableList, CommunicationService communicationService,
            ServiceType serviceXML) {
        stateToType = new HashMap<>();
        argumentOut = new HashMap<>();
        argumentState = new HashMap<>();
        name = action.getName();
        this.connection = communicationService;
        this.serviceXML = serviceXML;
        for (StateVariableType s : stateVariableList) {
            Type type = HelperUtils.findJavaTypeForDataType(s.getDataType());
            stateToType.put(s.getName(), type);
        }
        if (action.getArgumentList() != null) {
            for (ArgumentType a : action.getArgumentList().getArgument()) {
    ...
    public Response execute(Map<String, Object> arguments) throws UnsupportedOperationException, IOException {
        ...
        return getMessage(connection.sendData(serviceXML.getControlURL(),
                serviceXML.getServiceType() + "#" + this.getName(), message));
    }
